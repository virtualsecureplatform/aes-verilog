...
int main(int argc, char** argv) {
	...
	unsigned char charexpandedkey[128/8*(Nr+1)];
	aes.KeyExpansion(charkey,charexpandedkey);
	...
	for(int i = 0; i < 128/32*(Nr+1); i++){
		dut->expandedKey[i] = charexpandedkey[4*i];
		for(int j = 1; j < 4; j++) dut->expandedKey[i] += charexpandedkey[4*i+j]<<(8*j);
	}
	clock(dut,tfp);
